#THIS IS YOUR DEPLOYMENT KA FILE
...
model = load_model('chatbot_model.h5')#bitch
...
#Part 2 is the flask end

@app.route("/")
def home():
    return render_template("index.html")

@app.route("/outputwhatever",methods=["POST"])
def outputwhatever():
    input = request.form["userinput"]
    ints = predict_class(input, model)
    res = getResponse(ints, intents)

    return render_template("index.html", output_text = res)
